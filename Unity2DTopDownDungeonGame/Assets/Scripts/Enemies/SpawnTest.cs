...
public class SpawnTest : MonoBehaviour
{
    ...
    private GameObject instantiatedEnemyList;
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.T))
        {
            ...
            {
                Destroy(instantiatedEnemyList);
            }

            EnemyDetailsSO enemyDetails = randomEnemyHelperClass.GetItem();

            if (enemyDetails != null)
                instantiatedEnemyList = (Instantiate(enemyDetails.enemyPrefab, HelperUtilities.GetSpawnPositionNearestToPlayer(HelperUtilities.GetMouseWorldPosition()), Quaternion.identity));
        }
    }
}
